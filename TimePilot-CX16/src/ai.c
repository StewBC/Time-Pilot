...
void aiEndFrame() {
    ...
    if(activeStage < TIME_PERIOD3_1982) {
        if(!(frameCounter & 3)) {
            ...
            propState ^= 1;
            if(propState) {
                VERA.data0 = colorPaletteSky[activeStage] & 0xff; 
                VERA.data0 = colorPaletteSky[activeStage] >> 8;
                VERA.data0 = colorPaletteProps[activeStage] & 0xff;
                VERA.data0 = colorPaletteProps[activeStage] >> 8;
            } else {
                VERA.data0 = colorPaletteProps[activeStage] & 0xff;
                VERA.data0 = colorPaletteProps[activeStage] >> 8;
                VERA.data0 = colorPaletteSky[activeStage] & 0xff; 
    ...
    if(enemiesKilled > ENEMIES_TO_KILL_TO_CLEAR) {
        ...
        if(levelBossTimer >= 0) {
            if(--levelBossTimer < 0) {
                int8_t X = horizontalLaunchRayTable[playerAngle];
        ...
        if(TIME_PERIOD4_2001 != activeStage) {
            ...
            if(parachuteTimer >= 0) {
                if(--parachuteTimer == 0) {
                    int8_t X = playerAngle + ((aiRandom() & 3) - 2) & 31;
                    parachuteTimer--;
                    spawnX = launchPosX[X];
                    spawnY = launchPosY[X];
...
void aiEnemy(int8_t X) {
    int8_t eID = aiSpawnTimer & 7;
    // Move by player "thrust"
    aiAddVelocity(X, invPlayerAngle+VELOCITY_119);
    // Is the time right to think
    if(enemyID[eID] == X) {
        int8_t current, target, x, y, calculated = 0;
        ...
        if (current != enemyHeading[eID]) {
            int8_t A = (enemyHeading[eID] - current) & 31;
            // A >= 16 anti clock
            if(A & 16) {
                current--;
            } else {
        ...
        if(enemyWeapon[eID] >= 128 && !(exitGameMask & EXIT_PLAYER_DIED)) {
            ...
            if(target == activeFrame[X]) {
                int8_t A = (target - playerAngle) & 31;
                if(A >= 30 || A < 2) {
                ...
                if(!activeStage) {
                    ...
                    // In Periods 2, 3 or 4, Launch a missile?
                             ...
                             launchSide && (activeMinY[X] >= PLAYFIELDH*8-WEAPON_BORDER || activeMinX[X] >= PLAYFIELDW*8-WEAPON_BORDER)) {
                    spawnX = activeMinX[X] + 8;
                    spawnY = activeMinY[X] + 4;
    ...
    if((activeFlags[X] & ACTIVEFLAGS_CLIPMASK) == ACTIVEFLAGS_CLIPMASK) {
        if(activeLayer[X] == LAYER_PLAYER_BULLETS) {
            ...
            if(activeFlags[X] & (ACTIVEFLAGS_MULTIPURPOSE | ACTIVEFLAGS_TRACKED)) {
                ...
                // Actual enemy bullet
                int8_t mask = activeFlags[X] & ACTIVEFLAGS_ENEMYMASK;
...
void aiSpawnEnemy(){
    ...
    for(Y=ACTIVEFLAGS_ENEMYMASK; Y>=0 ; Y--) {
        if(enemyID[Y] > 127) {
            int8_t A = (((aiRandom() & 7) - 4) + playerAngle) & 31;
            spawnX = launchPosX[A];
            spawnY = launchPosY[A];
...
uint8_t aiTurnOnRay(uint8_t X) {
    ...
    // Get the X and Y and look up the beam it should ride
    x = ((activeMinX[X] >> 4) + 3) & 31;
    y = ((activeMinY[X] >> 4) + 3) & 31;
    target = rays[y][x];
